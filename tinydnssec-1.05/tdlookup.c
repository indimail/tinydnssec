...
addNSEC3(char *hashName)
{
	...
	while ((r = find(hashName, 0))) {
		...
		if (byte_equal(type, 2, DNS_T_NSEC3)) {
			if (!response_rstart(hashName, DNS_T_NSEC3, ttl))
				return 0;
			if (!response_addbytes(data + dpos, dlen - dpos))
				return 0;
			response_rfinish(RESPONSE_AUTHORITY);
		} else if (do_dnssec && byte_equal(type, 2, DNS_T_RRSIG) && dlen > dpos + 18 && byte_equal(data + dpos, 2, DNS_T_NSEC3)) {
			if (!response_rstart(hashName, DNS_T_RRSIG, ttl))
				return 0;
			if (!dobytes(18))
				return 0;
			if (!doname())
 ...
 */
	...
	if (algo != 1)
		return 0;				/* not found or unsupported algorithm / flags */
...
static int
addClosestEncloserProof(char *name, char *control, int includeWild)
{
	char           *q = name;
	char           *hashName = 0;
	int             r;
	...
	while (*q) {
		...
		while ((r = find(q, 0))) {
			...
			if (byte_equal(type, 2, DNS_T_HASHREF) && dlen > dpos) {
				if (!dns_packet_getname(data, dlen, dpos, &hashName))
					return 0;
				break;
			}
		}
	...
	if (!*q)
		return 0;
	if (includeWild && !addNSEC3Cover(q, control, 1))
		return 0;
	...
	for (;;) {
		...
		while ((r = find(control, 0))) {
			...
			if (byte_equal(type, 2, DNS_T_SOA))
				...
				flagns = 1;
			else if (byte_equal(type, 2, DNS_T_DNSKEY))
				flagsigned |= 1;
			else if (byte_equal(type, 2, DNS_T_RRSIG))
				flagsigned |= 2;
			else if (byte_equal(type, 2, DNS_T_NSEC3PARAM))
				flagsigned |= 4;
#else
	...
	for (;;) {
		...
		while ((r = find(wild, wild != q))) {
			...
			if (do_dnssec && byte_equal(type, 2, DNS_T_HASHREF) && dlen > dpos) {
				if (!dns_packet_getname(data, dlen, dpos, &nsec3))
					return 0;
			}
			if (byte_diff(type, 2, qtype) && byte_diff(qtype, 2, DNS_T_ANY) && byte_diff(type, 2, DNS_T_CNAME)
				&& (!do_dnssec || byte_diff(type, 2, DNS_T_RRSIG)))
				continue;
			if (byte_equal(type, 2, DNS_T_HASHREF) || byte_equal(type, 2, DNS_T_HASHLIST))
				continue;
			if (do_dnssec && byte_equal(type, 2, DNS_T_RRSIG) && dlen - dpos > 18) {
				...
				if (byte_diff(sigtype, 2, qtype) && byte_diff(qtype, 2, DNS_T_ANY) && byte_diff(sigtype, 2, DNS_T_CNAME))
					continue;
			...
			if (byte_equal(type, 2, DNS_T_NS) || byte_equal(type, 2, DNS_T_CNAME) || byte_equal(type, 2, DNS_T_PTR)) {
				...
				if (byte_equal(type, 2, DNS_T_CNAME) && byte_diff(qtype, 2, DNS_T_CNAME)) {
					if (!dns_domain_copy(&flagcname, d1))
						return 0;
				}
#endif
...
#endif
			else if (!response_addbytes(data + dpos, dlen - dpos))
				return 0;
		...
		for (i = 0; i < addr6num; ++i)
			if (i < 8) {
				if (!response_rstart(q, DNS_T_AAAA, addr6ttl))
					return 0;
				if (!response_addbytes(addr6[i], 16))
					return 0;
				response_rfinish(RESPONSE_ANSWER);
			}
  ...
  AUTHORITY:
		...
		if (!flaggavesoa) {
			...
			while ((r = find(control, 0))) {
				...
				if (!flaggavesoa && byte_equal(type, 2, DNS_T_SOA)) {
					...
					if (!response_rstart(control, DNS_T_RRSIG, ttl))
						return 0;
					if (!dobytes(18))
						return 0;
					if (!doname())
		...
		if (do_dnssec && flagsigned) {
			if (flagfound && nsec3) {	/* NODATA */
				...
				if (wild != q) {	/* Wildcard NODATA */
					if (!addClosestEncloserProof(q, control, 0))
						return 0;
				}
			} else {			/* NXDOMAIN, or query for NSEC3 owner name */
				if (!addClosestEncloserProof(q, control, 1))
					return 0;
			}
		}
	}
	...
	if (flagauthoritative && (aupos == anpos)) {
		...
		if (do_dnssec && wild != q && flagsigned) {	/* Wildcard answer */
			char           *nextCloser = q;
			while (nextCloser + *nextCloser + 1 < wild) {
				nextCloser += *nextCloser + 1;
			}
			if (!addNSEC3Cover(nextCloser, control, 0))
				return 0;
		}
#endif
		if (want(control, DNS_T_NS)) {
			...
			while ((r = find(control, 0))) {
				...
				if (byte_equal(type, 2, DNS_T_NS)) {
					...
					if (!response_rstart(control, DNS_T_DS, ttl))
						return 0;
					if (!response_addbytes(data + dpos, dlen - dpos))
						return 0;
					response_rfinish(RESPONSE_AUTHORITY);
...
#ifdef DNSSEC
			if (do_dnssec && !flagauthoritative && !have_ds) {
				addNSEC3(control);
			}
#endif
		}
	 ...
	 * Add EDNS0 OPT RR 
	 */
		if (!response_rstart("", DNS_T_OPT, 1 << 15))
			return 0;
	...
	while (bpos < arpos) {
		...
		if (byte_equal(x, 2, DNS_T_NS) || byte_equal(x, 2, DNS_T_MX)) {
			if (byte_equal(x, 2, DNS_T_NS)) {
				if (!dns_packet_getname(response, arpos, bpos, &wantAddr))
					return 0;
			} else if (!dns_packet_getname(response, arpos, bpos + 2, &wantAddr))
				return 0;
			...
			if (want(wantAddr, DNS_T_A)) {
				...
				while ((r = find(wantAddr, 0))) {
					...
					if (byte_equal(type, 2, DNS_T_A)) {
						if (!response_rstart(wantAddr, DNS_T_A, ttl))
							return 0;
						if (!dobytes(4))
							return 0;
						response_rfinish(RESPONSE_ADDITIONAL);
					} else if (byte_equal(type, 2, DNS_T_AAAA)) {
						if (!response_rstart(wantAddr, DNS_T_AAAA, ttl))
							return 0;
						if (!dobytes(16))
							return 0;
						response_rfinish(RESPONSE_ADDITIONAL);
						...
						if (!response_rstart(wantAddr, DNS_T_RRSIG, ttl))
							return 0;
						if (!dobytes(18))
							return 0;
						if (!doname())
			...
			if (want(d1, DNS_T_A)) {
				...
				while (r = find(d1, 0)) {
					...
					if (byte_equal(type, 2, DNS_T_A)) {
						...
						if (!response_rstart(d1, DNS_T_AAAA, ttl))
							return 0;
						if (!dobytes(16))
							return 0;
						response_rfinish(RESPONSE_ADDITIONAL);
...
int
respond(char *q, char qtype[2], char ip[16])
{
